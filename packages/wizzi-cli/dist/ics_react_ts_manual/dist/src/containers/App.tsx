/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-cli\dist\ics_react_ts_manual\.wizzi\src\containers\App.tsx.ittf
...
class App extends React.Component<Props, State> {
    ...
    static getDerivedStateFromProps(props: Props, state: State) {
        if (props.currentPacki && props.currentPacki.id !== state.packiStoreId) {
            ...
            if (files) {
                ...
                return {
                        ...
                        isWizziJobWaiting: (((fileEntries.filter(e => 
        ...
        if (props.jobGeneratedArtifacts && props.jobGeneratedArtifacts !== state.jobGeneratedArtifacts) {
            const notGenerated = entryArrayToPacki(state.fileEntries.filter(e => 
                !e.item.generated
            ));
    ...
    componentDidMount() {
        ...
        this.props.dispatchInitPacki(this.props.preferences);
    ...
    componentDidUpdate(_: Props, prevState: State) {
        ...
        Object.keys(diff).forEach(// console.log('componentDidUpdate.changed', k, diff[k].kind);
        (k) => {
            if (diff[k].kind === '+' || diff[k].kind === '-') {
                didFilesChange = true;
            }
            else {
                if (diff[k].b && (diff[k].b as FileSystemEntry['item'])
                    .virtual) {
                    didFilesChange = true;
                }
            }
    ...
    _initializePackiSession = // lots of inits
    async () => 
        this.setState({
    ...
    _generateArtifactNotDebounced = () => {
        ...
        if (focusedEntry) {
            ...
            this.props.dispatchGenerateArtifact(focusedEntry.item.path, entryArrayToPacki(this.state.fileEntries.filter(e => 
                e.item.path.endsWith('.ittf')
            )))
        }
    };
    ...
    render() {
        return  (
            <MuiThemeProvider theme={THEME}>
                <EditorView params={this.state.params}
...
export default connect<StateProps, DispatchProps>(mapStateToProps, mapDispatchToProps)(withPreferences(App))
