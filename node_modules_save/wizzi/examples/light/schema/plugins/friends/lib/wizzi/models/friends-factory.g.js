/*
    ...
    utc time: Mon, 18 Jan 2021 15:50:14 GMT
*/
'use strict';
/**
     friends WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var friendsschema = require('./friends-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var friendsmodel = new friendsmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var friendsmodelType = friendsschema[rootNode.n];
            if (!friendsmodelType) {
                var maptag = friendsschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    friendsmodelType = friendsschema[maptag];
                }
                if (!friendsmodelType) {
                    var error = new errors.WizziModelLoadError('In friends Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: friends', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            friendsmodel = new friendsmodelType(rootNode.v);
            friendsmodel.loadHistory = mTree.loadHistory;
            friendsmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                friendsmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn friends Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new friendsschema.friendsContext();
        friendsmodel.wzInitialize(ctx);
        friendsmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In friends Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && friendsmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(friendsmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        friendsmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && friendsmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, friendsmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'friends' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'friends' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema friends from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
