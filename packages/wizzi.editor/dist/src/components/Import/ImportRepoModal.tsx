/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi.editor\.wizzi\src\components\Import\ImportRepoModal.tsx.ittf
...
export default class ImportRepoModal extends React.PureComponent<Props, State> {
        ...
        _handleImportRepoClick = async (e: React.FormEvent<HTMLFormElement>) => {
            ...
            let didFail = false;
            try {
                const IMPORT_API_URL = `${this.props.packierURL}/git`;
                const params: { 
                    repo: string;
                    subpath?: string;
                    branch?: string;
                ...
                ;
                packiId = await res.text();
                ;
                if (this.props.visible) {
                    if (res.ok) {
                        this.props.preventRedirectWarning();
                        window.location.href = `/${packiId}`;
                    }
                    else {
                        didFail = true;
                    }
                     ...
                     onSubmit={this._handleImportRepoClick}>
                        <p
                         className={!isError ? css(styles.paragraph) : css(styles.errorParagraph)}>
                            {
                                !isError ? 'Import a Wizzi project from a Git repository.' : 'An error occurred during import. This could be because the data provided was invalid, or because the repository referenced is not a properly formatted Wizzi project.'
                            }
                        </p>
                        {
                            advanced ?  (
