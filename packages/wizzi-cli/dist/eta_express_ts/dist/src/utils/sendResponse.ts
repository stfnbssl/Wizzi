/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-cli\dist\eta_express_ts\.wizzi\src\utils\sendResponse.ts.ittf
...
;
export const sendFailure = (res: Response, error: HttpException, status: number) => {
...
export function sendPromiseLikeResult<T>(res: Response, message: PromiseLike<T>) {
    message.then(// log 'sendPromiseLikeResult.ok', result
    (result: T) => 
        sendSuccess(res, result)
    )
}
