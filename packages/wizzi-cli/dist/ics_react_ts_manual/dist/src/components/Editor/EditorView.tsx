/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-cli\dist\ics_react_ts_manual\.wizzi\src\components\Editor\EditorView.tsx.ittf
...
type EditorProps = authTypes.AuthProps & // loadingMessage: string | undefined;
// 
{ 
    classes: any;
...
export type Props = prefTypes.PreferencesContextType & EditorProps & { 
    loggedUser?: appTypes.LoggedUser;
};
type ModalName = 'auth' | 'packi-manager' | 'github-commit' | 'github-create' | 'edit-info' | 'shortcuts' | 'previous-saves';
type BannerName = 'connected' | 'disconnected' | 'reconnect' | 'autosave-disabled' | 'slow-connection';
type SplitViewKind = 'left' | 'right' | 'both';
type State = // lintErrors: Annotation[];
{ 
...
class EditorView extends React.Component<Props, State> {
    static getDerivedStateFromProps(props: Props, state: State) {
        if (props.entry !== state.previousEntry) {
            ...
            if (entry && (!previousEntry || entry.item.path !== previousEntry.item.path) && // When an empty markdown file is opened, switch to edit mode
            (entry.item.type === 'file' && entry.item.path.endsWith('.md') && !entry.item.asset && !(entry as filelistTypes.TextFileEntry).item.content)) {
        ...
        return  (
            <ContentShell>
                ...
                <React.Fragment>
                    ...
                    <EditorToolbar creatorUsername={this.props.creatorUsername}
                        ...
                        <div className={css(styles.editorAreaOuter)}>
                            <LayoutShell>
                                     ...
                                     &&  (
                                        <LazyLoad load={():  Promise<typeof import('./MonacoEditor')> => {
                                            ...
                                            return MonacoEditorPromise.then(({
                                                    ...
                                                    console.log(err);
                                                    alert('Failed to load Monaco Editor. See console error.');
                                                }
                                                )
                                            ;
                                        }
                                        }>
                                            {
                                                ({
                                                    ...
                                                    if (entry && entry.item.type === 'file') {
                                                        if (entry.item.asset) {
                                                            return  (
                                                                <AssetViewer entry={(entry as any) as filelistTypes.AssetFileEntry} />
                                                                )
                                                            ;
                                                        }
                                                        const {
                                ...
                                }
                                {
                                    ((this.state.splitViewKind == 'both' || this.state.splitViewKind == 'right') && generatedArtifact) && generatedArtifact.artifactContent ?  (
                                        <GeneratedView generatedContent={generatedArtifact.artifactContent} generatedSourcePath={generatedArtifact.sourcePath} splitViewKind={this.state.splitViewKind} />
                                        )
                                     : generatedArtifact && generatedArtifact.errorLines ?  (
                                            <GenerationErrors errorName={generatedArtifact.errorName}
                                ...
                                {
                                    jobError ?  (
                                        <GenerationErrors errorName={jobError.errorName}
                                            errorLines={[]}
                            ...
                            </LayoutShell>
                            {
                                preferences.panelsShown ?  (
                                    <EditorPanels onShowErrorPanel={this._showErrorPanel}
                    ...
                    <EditorFooter loggedUid={preferences.loggedUid}
                     ...
                     />
                    {
                        loggedUser
                         &&  (
                            <ModalDialog title="Manage your packies" visible={currentModal === 'packi-manager'} onDismiss={this._handleHideModal}>
                                <PackiManager onClose={this._handleHideModal} />
                            </ModalDialog>
                            )
                        
                    }
                    <ModalDialog visible={currentModal === 'previous-saves'} title="Previous saves" onDismiss={this._handleHideModal}>
                        <PreviousSaves saveHistory={saveHistory} />
                    </ModalDialog>
                    <ModalDialog title="Shortcuts" visible={currentModal === 'shortcuts'} onDismiss={this._handleHideModal}>
                        <KeyboardShortcuts />
                    </ModalDialog>
                    {
                        currentPacki && currentPacki.localPackiData
                         &&  (
                            <ModalDialog title="Commit/push git package" visible={currentModal === 'github-commit'} onDismiss={this._handleHideModal}>
                                <EditorForm title="Commit/push git package"
                                    action="Done"
                                    visible={true}
                                    onDismiss={this._handleHideModal}
                                    onSubmit={// TODO this._handleCommitGitRepository(values['owner'], values['repoName'], values['branch']);
                                    values => 
                    ...
                    }
                    {
                        currentPacki && currentPacki.localPackiData
                         &&  (
                            <ModalDialog title="Create git package" visible={currentModal === 'github-create'} onDismiss={this._handleHideModal}>
                                <EditorForm title="Create git package"
                                    action="Confirm"
