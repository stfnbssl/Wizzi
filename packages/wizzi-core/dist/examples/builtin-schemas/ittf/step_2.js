/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-core\.wizzi\ittf\examples\builtin-schemas\ittf\step_2.js.ittf
*/
'use strict';
/**
...
var Schemas_ittf_step_2 = function(step_callback) {
    ...
    loadModelAndGenerateArtifact(path.join(__dirname, 'ittf', 'basic.ittf.ittf'), {}, "ittf/document", function(err, artifactText) {
        ...
        heading2('Ittf builtin schema - artifact ittf/document');
        printValue('ittf/document', artifactText);
        loadAndTransformModel(path.join(__dirname, 'ittf', 'basic.ittf.ittf'), {}, "ittf/html-pretty", function(err, artifactText) {
            if (err) {
                console.log('err', err);
                throw new Error(err.message);
            }
            heading2('Ittf builtin schema - transformation ittf/html-pretty');
            printValue('ittf/html-pretty', artifactText);
        });
    });
};
